...
class Guard:
    ...
    def calculateSleepTimes(self):
        ...
        max_val = max(sleepTimes)
        return (sleepTimes.index(max_val), max_val)
...
records.sort(key=lambda x: x.date + str(x.hour) + str(x.minute).zfill(2), reverse=False)
...
for record in records:
    ...
    if matches:
        ...
        print("Applying "+str(record)+" to guard with id ="+str(guard.id))
        guard.apply(record)
if guard != None:
    guard.finish()

for i, guard in guards.items():
    print("#"+str(guard.id) + " Sleap time = "+str(guard.sleepTime())+ " Max: "+str(guard.calculateSleepTimes()[1])+" in "+str(guard.calculateSleepTimes()[0]))
...
print(maxSleapyGuard.id * maxSleapyGuard.calculateSleepTimes()[0])

print("part2")

i, maxSleapyGuard = max(guards.items(), key=lambda g: g[1].calculateSleepTimes()[1])
print(maxSleapyGuard.id)
print(maxSleapyGuard.calculateSleepTimes())
print(maxSleapyGuard.id * maxSleapyGuard.calculateSleepTimes()[0])
